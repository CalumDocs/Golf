...
function Badge({ category }) {
  ...
  return (
    <span className="px-2 py-0.5 text-xs rounded-full" style={{ backgroundColor: color, color: darkText ? THEME.text : "#FFFFFF" }}>{category}</span>
  );
}

function StickyFooter({ canContinue, onContinue, note, cta = "Continue" }) {
  return (
    <div className="fixed inset-x-0 bottom-0 p-3" style={{ backgroundColor: THEME.panel, borderTop: `1px solid ${THEME.border}` }}>
      <div className="flex items-center gap-3 max-w-xl mx-auto">
        <div className="flex-1 text-xs" style={{ color: THEME.subtext }}>{note}</div>
        <button onClick={onContinue} disabled={!canContinue} className={`px-4 py-2 rounded-2xl ${canContinue ? "" : "opacity-60"}`} style={{ backgroundColor: THEME.gold, color: THEME.header }}>{cta}</button>
      </div>
    </div>
  );
}
...
function runInlineTests() {
  try {
    ...
    const core = PACKAGES.find(p => p.id === "core");
    const m = packageMetrics(core);
    console.assert(Math.abs(m.cpc - (core.price / core.credits)) < 1e-9, "Cost per credit calc failed");
    console.assert(m.sig > m.sel && m.sel > m.cla, "Round cost ordering should be Signature then Select then Classic");
  } catch (e) {
...
export default function App() {
  ...
  useEffect(() => {
    if (step === STEPS.BOOK && !bookingDraft.date && !bookingDraft.time) {
      const d = new Date();
  ...
  function Page({ title, subtitle, children }) {
    return (
      <div className="min-h-screen pb-28" style={{ background: THEME.bg }}>
        <header className="p-4" style={{ background: THEME.header, color: "#FFFFFF", borderBottom: `3px solid ${THEME.gold}` }}>
        ...
        <main className="p-4 max-w-xl mx-auto" style={{ color: THEME.text }}>{children}</main>
      </div>
    );
  }
    ...
    return (
      <Page title="Play one hundred Yorkshire courses with one flexible membership" subtitle="Choose a travel radius pick your favourites spend credits by category and upgrade any time">
        <div className="grid gap-3 text-sm">
          <div className="p-4 rounded-2xl" style={{ background: THEME.panel, border: `1px solid ${THEME.border}` }}>
            <div className="font-semibold">How it works</div>
            <p className="mt-1" style={{ color: THEME.subtext }}>Set your radius then select courses across Signature Select and Classic. Each category has its own credit bank so everyone gets fair access to the most popular venues.</p>
          </div>
          <div className="p-4 rounded-2xl" style={{ background: THEME.panel, border: `1px solid ${THEME.border}` }}>
            <div className="font-semibold">Why join</div>
            <ul className="list-disc pl-5 mt-1" style={{ color: THEME.subtext }}>
        ...
        <button className="w-full mt-6 py-3 rounded-2xl" style={{ background: THEME.gold, color: THEME.header }} onClick={() => setStep(STEPS.SIGNUP)}>Get started</button>
      </Page>
    );
  }
    ...
    return (
      <Page title="Create your profile" subtitle="We use this to tailor courses and travel distance. You can update later">
        <div className="space-y-3">
          <label className="text-sm" style={{ color: THEME.subtext }}>Full name</label>
          <input ref={nameRef} className="w-full border rounded-xl p-3" style={{ borderColor: THEME.border, background: THEME.panel }} placeholder="Name" defaultValue="" />
          <label className="text-sm" style={{ color: THEME.subtext }}>Email</label>
          <input ref={emailRef} className="w-full border rounded-xl p-3" style={{ borderColor: THEME.border, background: THEME.panel }} placeholder="Email" defaultValue="" />
          <label className="text-sm" style={{ color: THEME.subtext }}>Date of birth</label>
          <input ref={dobRef} type="date" className="w-full border rounded-xl p-3" style={{ borderColor: THEME.border, background: THEME.panel }} />
          <label className="text-sm" style={{ color: THEME.subtext }}>Postcode</label>
          <input ref={postcodeRef} className="w-full border rounded-xl p-3" style={{ borderColor: THEME.border, background: THEME.panel }} placeholder="Postcode" defaultValue={profile.postcode} />
          <div>
            <label className="text-sm" style={{ color: THEME.subtext }}>Handicap</label>
            <input ref={handicapRef} type="number" min={0} max={54} className="w-full border rounded-xl p-3" style={{ borderColor: THEME.border, background: THEME.panel }} defaultValue={profile.handicap} />
            <p className="text-xs mt-1" style={{ color: THEME.subtext }}>Suggested amateur handicap is set to 18 by default</p>
          </div>
        </div>
        <div className="h-10" />
        <StickyFooter canContinue={true} onContinue={() => {
          setProfile(p => ({ 
            ...
            postcode: postcodeRef.current?.value || p.postcode, 
            handicap: Number(handicapRef.current?.value || p.handicap) 
          })); 
          setStep(STEPS.RADIUS); 
        }} note="Next choose your travel radius" />
      </Page>
    );
  }

  if (step === STEPS.RADIUS) {
    ...
    return (
      <Page title="Choose your travel radius" subtitle="We show courses inside your chosen distance from home">
        ...
        <p className="text-sm mb-2" style={{ color: THEME.subtext }}>Courses in range right now {byDistance.length}</p>
        <div className="divide-y rounded-2xl border mb-6" style={{ borderColor: THEME.border, background: THEME.panel }}>
          {byDistance.map(c => (
            <div key={c.id} className="p-3 flex items-center justify-between">
              <div>
                <div className="font-medium flex items-center gap-2">{c.name} <Badge category={c.category} /></div>
                <div className="text-xs" style={{ color: THEME.subtext }}>{c.city_area || "Yorkshire"} • {c.distance.toFixed(1)} miles</div>
              </div>
            </div>
          ))}
          {byDistance.length === 0 && (
            <div className="p-4 text-sm" style={{ color: THEME.subtext }}>No courses in this radius. Try a larger distance</div>
          )}
        </div>
        <StickyFooter canContinue={true} onContinue={()=>setStep(STEPS.FREQUENCY)} note="Next pick how often you play" />
      </Page>
    );
  }
      ...
      <Page title="How often do you play" subtitle="Pick the package that fits your year. We show price credits and estimated round value">
        <div className="grid gap-3">
          {cards.map(c => {
            ...
            return (
              <button key={c.id} onClick={()=>setPackageId(c.id)} className={`text-left p-4 rounded-2xl border ${selected?"border-green-600 bg-green-50":"bg-white"}`} style={{ borderColor: THEME.border }}>
                ...
                <div className="text-sm" style={{ color: THEME.subtext }}>{p.name} • {p.credits} credits • £{p.price} • £{m.cpc.toFixed(2)} per credit</div>
                <div className="text-xs mt-1" style={{ color: THEME.subtext }}>Est round cost Signature £{m.sig.toFixed(2)} Select £{m.sel.toFixed(2)} Classic £{m.cla.toFixed(2)}</div>
              </button>
            );
          })}
        </div>
        <div className="mt-4 text-xs" style={{ color: THEME.subtext }}>Your starting banks Signature {banks.Signature} Select {banks.Select} Classic {banks.Classic}</div>
        <div className="h-10" />
        <StickyFooter canContinue={true} onContinue={()=>setStep(STEPS.ALLOCATE)} note="Next allocate rounds to courses inside your radius" />
      </Page>
    );
  }

  if (step === STEPS.ALLOCATE) {
    ...
    const section = (cat) => (
      <div className="mb-4" key={cat}>
        <h3 className="text-sm font-semibold mb-2 flex items-center gap-2"><Badge category={cat} /> <span style={{ color: THEME.subtext }}>Courses in range</span></h3>
    ...
    return (
      <Page title="Pick your courses and spend credits" subtitle="Each category has its own bank. Plus and minus adjust rounds. Caps prevent overuse">
        ...
        {section("Classic")}
        <div className="h-14" />
        <StickyFooter canContinue={true} onContinue={()=>setStep(STEPS.UPSELL)} note="You can adjust allocations later from the dashboard" />
      </Page>
    );
  }

  if (step === STEPS.UPSELL) {
    return (
      <Page title="Boost your season" subtitle="Optional extras that improve value for big days out">
        <div className="grid gap-3">
          {ADD_ONS.map(x => {
            ...
            return (
              <button key={x.id} onClick={()=>setSelectedAddOns(s=>({ ...s, [x.id]: !s[x.id] }))} className={`text-left p-4 rounded-2xl border ${active?"bg-green-50 border-green-600":"bg-white"}`} style={{ borderColor: THEME.border }}>
                <div className="font-medium flex items-center justify-between">
                  <span>{x.name}</span>
                  {x.price>0 && <span>£{x.price}</span>}
                </div>
                <div className="text-sm" style={{ color: THEME.subtext }}>{x.desc}</div>
              </button>
            );
          })}
        </div>
        ...
        <div className="h-10" />
        <StickyFooter canContinue={true} onContinue={()=>setStep(STEPS.SUMMARY)} note="Review your membership then confirm" />
      </Page>
    );
  }
  ...
  if (step === STEPS.SUMMARY) {
    ...
    return (
      <Page title="Your membership" subtitle="This is a demo. Nothing will be submitted or charged">
        <div className="p-4 rounded-2xl border mb-4" style={{ background: THEME.panel, borderColor: THEME.border }}>
          <div className="font-semibold">Package</div>
          <div className="text-sm" style={{ color: THEME.subtext }}>{pkgObj.name} • {pkgObj.credits} credits • £{pkgObj.price}</div>
          <div className="text-xs mt-1" style={{ color: THEME.subtext }}>Banks Signature {banks.Signature} Select {banks.Select} Classic {banks.Classic}</div>
        </div>
        <div className="p-4 rounded-2xl border mb-4" style={{ background: THEME.panel, borderColor: THEME.border }}>
          <div className="font-semibold mb-1">Add ons</div>
          {chosenAddOns.length === 0 ? (
            <div className="text-sm" style={{ color: THEME.subtext }}>No add ons selected</div>
          ) : (
        ...
        <button className="w-full py-3 rounded-2xl" style={{ background: THEME.gold, color: THEME.header }} onClick={()=>setStep(STEPS.DASHBOARD)}>Confirm</button>
      </Page>
    );
  }
    ...
    return (
      <Page title="Confirm your round" subtitle="Pick a date and time then add optional extras">
        <div className="p-4 rounded-2xl border mb-4" style={{ background: THEME.panel, borderColor: THEME.border }}>
          ...
          <div className="text-sm" style={{ color: THEME.subtext }}>{c?.city_area}</div>
          <div className="mt-3 grid gap-2">
            <input type="date" className="border rounded-xl p-3" style={{ borderColor: THEME.border, background: THEME.panel }} value={bookingDraft.date} onChange={e=>setBookingDraft({...bookingDraft, date:e.target.value})} />
            <input type="time" className="border rounded-xl p-3" style={{ borderColor: THEME.border, background: THEME.panel }} value={bookingDraft.time} onChange={e=>setBookingDraft({...bookingDraft, time:e.target.value})} />
          </div>
        </div>
        <div className="p-4 rounded-2xl border mb-4" style={{ background: THEME.panel, borderColor: THEME.border }}>
        ...
        <button className="w-full py-3 rounded-2xl mb-3" style={{ background: THEME.gold, color: THEME.header }} onClick={confirmBooking}>Confirm booking</button>
        <button className="w-full py-3 rounded-2xl border" onClick={()=>setStep(STEPS.DASHBOARD)}>Cancel</button>
      </Page>
    );
  }

  const greeting = profile.name ? `Welcome back ${profile.name}` : "Welcome back Golfer";
  const nextRoundObj = nextRound;
  const nextCourse = nextRoundObj ? COURSES.find(c => bookings.find(b => new Date(b.whenISO).getTime() === nextRoundObj.date.getTime())?.courseId) : null;
  const lastCourse = lastRound ? COURSES.find(c => c.id === lastRound.courseId) : null;

  return (
    <Page title={greeting} subtitle="Book rounds track credits collect stamps">
      <section className="mb-4 p-4 rounded-2xl border" style={{ background: THEME.panel, borderColor: THEME.border }}>
        <div className="font-semibold mb-1">Your next round</div>
        {!nextRoundObj && <div className="text-sm" style={{ color: THEME.subtext }}>No rounds booked yet</div>}
        {nextRoundObj && (
          <div className="text-sm">
            Starts in {nextRoundObj.hours} hours {nextRoundObj.minutes} minutes
            <div className="text-xs" style={{ color: THEME.subtext }}>{nextRoundObj.date?.toLocaleString?.()}</div>
            {canOrderBalls && (
      ...
      <section className="mb-4 p-4 rounded-2xl border" style={{ background: THEME.panel, borderColor: THEME.border }}>
        ...
        {COURSES.filter(c => (alloc[c.id] || 0) > 0).length === 0 && (
          <div className="text-sm" style={{ color: THEME.subtext }}>You have not allocated any rounds yet</div>
        )}
        <div className="grid gap-2">
          {COURSES.filter(c => (alloc[c.id] || 0) > 0).map(c => (
            <div key={c.id} className="flex items-center justify-between p-3 rounded-xl border" style={{ borderColor: THEME.border }}>
              <div>
                <div className="font-medium flex items-center gap-2">{c.name} <Badge category={c.category} /></div>
                <div className="text-xs" style={{ color: THEME.subtext }}>Rounds remaining {alloc[c.id]}</div>
              </div>
              <button className="px-3 py-2 rounded-xl" style={{ background: THEME.gold, color: THEME.header }} onClick={()=>openBooking(c.id)}>Book</button>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-4 p-4 rounded-2xl border" style={{ background: THEME.panel, borderColor: THEME.border }}>
        ...
        <div className="text-xs mt-2" style={{ color: THEME.subtext }}>Need flexibility later Use buy more credits</div>
        <button className="mt-2 px-4 py-2 rounded-2xl" style={{ background: THEME.gold, color: THEME.header }} onClick={()=>{
          const order = ["starter","core","enthusiast","elite"];
          const idx = order.indexOf(packageId);
          const next = order[Math.min(order.length - 1, idx + 1)];
          if (next !== packageId) setPackageId(next);
        }}>Buy more credits</button>
      </section>

      <section className="mb-24 p-4 rounded-2xl border" style={{ background: THEME.panel, borderColor: THEME.border }}>
        <div className="font-semibold mb-2">Golf stamps</div>
        <div className="text-sm" style={{ color: THEME.subtext }}>Play new courses to collect stamps. Ten stamps earns a free round at a Classic. Rewards will be activated later.</div>
      </section>

      <div className="fixed inset-x-0 bottom-0 p-3" style={{ background: THEME.panel, borderTop: `1px solid ${THEME.border}` }}>
        <button className="w-full py-3 rounded-2xl border" onClick={()=>setStep(STEPS.ALLOCATE)}>Adjust allocations</button>
      </div>
    </Page>
  );
}
